...
MAC_HARD0 = '80:aa:aa:aa:aa:10'
MAC_HARD1 = '12:34:56:78:9a:bc'

IP_HARD0 = '170.170.170.0'
IP_HARD1 = '170.170.170.170'

IP_DST_HARD = '170.170.170.10'
...
def write_frame(iface, f):
  ...
  if(cnt == 0):
    iface = 0
    data = b'\xff\xff\xff\xff\xff\xff\x11\x11\x11\x11\x11\x11\x09\x15\x01\x00\x00\x00\x00\xff\xaa\xaa\xaa\xa0\xaa\xaa\xaa\xaa'
  elif cnt == 1:
    iface = 0
    data = b'\xff\xff\xff\xff\xff\xff\x11\x11\x11\x11\x11\x11\x09\x15\x01\x00\x00\x00\x00\xff\xbb\xbb\xbb\xb0\xbb\xbb\xbb\xbb'
  elif cnt == 2:
    iface = 0
    data = bytes(Ether(src=MAC_HARD1, dst=MAC_HARD0) / ARP(op='is-at', hwsrc=MAC_HARD1, psrc=IP_HARD1, hwdst=MAC_HARD0, pdst=IP_HARD0))
  elif cnt == 3:
    iface = 0
    data = bytes(Ether(src=MAC_TESTER0, dst=MAC_DUT0) / IP(src=IP_TESTER0, dst=IP_DST_HARD, id=0xb01a, ttl=64) / UDP(sport=7, dport=7) / b'hello, 00009')
  else:
    return 
  f.show()
  cnt += 1
...
write_frame(1, Ether(src=MAC_TESTER1, dst=MAC_DUT1) /
            IP(src=IP_TESTER1, dst=IP_TEST_DST_NO_MAC) /
            UDP(sport=7, dport=7) / b'hello, 00003')

# Simple IP forwarding test (no route).
write_frame(1, Ether(src=MAC_TESTER1, dst=MAC_DUT1) /
            IP(src=IP_TESTER1, dst=IP_TEST_DST_NO_ROUTE) /
