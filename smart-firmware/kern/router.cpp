...
static uint32_t const size_addr1 = 0x7FE;
static uint32_t const size_addr2 = 0x7FF;
...
uint32_t receive(uint8_t *buffer, uint8_t *src_mac, uint8_t *dst_mac, int *if_index)
{
    ...
    uint8_t buf = 0;
    ptr = (volatile uint8_t *)(read_start + (src << width) + size_addr1);
    uint8_t length1 = *ptr;
    ptr = ptr + 1;
    uint8_t length2 = *ptr;
...
void send(int if_index, const uint8_t *buffer, uint32_t length, uint32_t dst, const uint8_t *dst_mac)
{
    ...
    *ptr = buf;
    ...
    length = length + 14;
    ptr = (volatile uint32_t*)(read_start + (dst << width) + size_addr1);
